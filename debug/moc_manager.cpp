/****************************************************************************
** Meta object code from reading C++ file 'manager.hpp'
**
** Created: Tue 2. Oct 20:43:14 2012
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../src/manager.hpp"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'manager.hpp' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Manager[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       9,    8,    8,    8, 0x08,
      25,    8,    8,    8, 0x08,
      46,   42,    8,    8, 0x08,
      66,   63,    8,    8, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Manager[] = {
    "Manager\0\0updateOptions()\0loadGlobalData()\0"
    "url\0sortURL(QString)\0ID\0"
    "checkConversation(QString)\0"
};

const QMetaObject Manager::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_Manager,
      qt_meta_data_Manager, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Manager::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Manager::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Manager::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Manager))
        return static_cast<void*>(const_cast< Manager*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int Manager::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: updateOptions(); break;
        case 1: loadGlobalData(); break;
        case 2: sortURL((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 3: checkConversation((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
