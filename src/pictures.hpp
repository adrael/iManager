...
public:
    ...
    PICTURES(QWidget * parent = 0, database * db = 0, const QString & path = "", Options * opt = 0) : QWidget(parent) {
        ...
        this -> searchPicture -> setVisible(this -> appOptions -> SearchBarPictures);
        ...
        this -> sortPicture -> setCurrentIndex(this -> appOptions -> FirstLastPictures ? 0 : 1);
        ...
        this -> scene = new QGraphicsScene(0, 0, 400, 480, this);
        this -> scene -> setBackgroundBrush(this -> appOptions -> BGColorPictures == "" ? this -> palette().window() : QColor(this -> appOptions -> BGColorPictures).toRgb());
        this -> scene -> addItem(widget);
        //this -> scene -> setFrameStyle(QFrame::NoFrame);
    ...
    void displayDefaultView() {
        ...
        this -> bigPic -> setIconSize(QSize(350, 350));
        this -> bigPic -> setIcon(QIcon(QPixmap(this -> appPath + "/utils/icons/nobody.png").scaled(350, 350, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
    }
...
public slots:
    ...
    void loadPicturesData(const int & = 0) {
        if(!this -> PicturesTreeList -> columnCount() > 0) {
